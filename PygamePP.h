...
namespace PG {
    ...
    struct Color {
        ...
        Color(uint32_t hexColor, unsigned char alpha = 255) {
            r = (hexColor >> 16) & 0xFF;
            g = (hexColor >> 8) & 0xFF;
            b = hexColor & 0xFF;
            a = alpha;
        ...
        Color(const SDL_Color& sdlColor) : r(sdlColor.r), g(sdlColor.g), b(sdlColor.b), a(sdlColor.a) {}

        operator SDL_Color() const {
           return SDL_Color{ r, g, b, a };
        }

        Color& operator=(const SDL_Color& sdlColor) {
            r = sdlColor.r;
            g = sdlColor.g;
            b = sdlColor.b;
            a = sdlColor.a;
    ...
    bool poll_event(Event& pg_event) {
        ...
        if (SDL_PollEvent(&sdl_event)) {
            ...
            if (sdl_event.type == SDL_KEYDOWN || sdl_event.type == SDL_KEYUP) {
                pg_event.key = sdl_event.key.keysym.scancode;
            }
            if (sdl_event.type == SDL_MOUSEBUTTONDOWN) {
                pg_event.button = sdl_event.button.button;
            }
    ...
    public:
      ...
      static inline bool game_active = false;
    ...
    public:
        Window(std::string title, int w, int h) : running(true), width(w), height(h) {
        ...
        void set_blend_mode(PG::BlendMode mode) {
        ...
        void fill_rect(const PG::Rect& rect, SDL_Color color) {
            SDL_Rect sdl_r = rect.to_sdl();
            SDL_SetRenderDrawColor(renderer, color.r, color.g, color.b, color.a);
            SDL_RenderFillRect(renderer, &sdl_r);
        }
